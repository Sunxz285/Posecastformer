...
def p_mpjpe(predicted, target):
    ...
    if np.any(np.isnan(predicted)) or np.any(np.isnan(target)):
        print("Warning: NaN values detected in input, using MPJPE instead")
        return mpjpe(predicted, target)

    if np.any(np.isinf(predicted)) or np.any(np.isinf(target)):
        print("Warning: Inf values detected in input, using MPJPE instead")
        return mpjpe(predicted, target)
    ...
    # Stronger numerical stability check
    eps = 1e-6  # increased epsilon value
    normX = np.maximum(normX, eps)
    normY = np.maximum(normY, eps)

    # Check if point cloud is too concentrated (close to a single point)
    if np.any(normX < eps) or np.any(normY < eps):
        print("Warning: Degenerate point cloud detected, using MPJPE instead")
        return mpjpe(predicted, target)

    X0 /= normX
    Y0 /= normY

    # Ensure the matrix condition number is not too poor
    H = np.matmul(X0.transpose(0, 2, 1), Y0)

    # Add regularization to improve numerical stability
    H += np.eye(H.shape[-1]) * 1e-8

    # Multiple attempts at SVD to increase convergence chance
    max_attempts = 3
    for attempt in range(max_attempts):
        try:
            ...
            if attempt == max_attempts - 1:  # Last attempt
                print(f"Warning: SVD did not converge after {max_attempts} attempts, using MPJPE instead")
                return mpjpe(predicted, target)
            else:
                # Add stronger regularization and retry
                H += np.eye(H.shape[-1]) * 1e-6
                continue
    ...
    try:
        ...
        if np.any(np.isnan(detR)) or np.any(np.isinf(detR)):
            print("Warning: Invalid determinant detected, using MPJPE instead")
            return mpjpe(predicted, target)

        sign_detR = np.sign(np.expand_dims(detR, axis=1))
        # Ensure sign is valid
        sign_detR = np.clip(sign_detR, -1, 1)
        ...
        if np.any(np.isnan(R)) or np.any(np.isinf(R)):
            print("Warning: Invalid rotation matrix, using MPJPE instead")
            return mpjpe(predicted, target)
        ...
        if np.any(np.isnan(a)) or np.any(np.isinf(a)) or np.any(a < 0):
            print("Warning: Invalid scale factor, using MPJPE instead")
            return mpjpe(predicted, target)
        ...
        if np.any(np.isnan(t)) or np.any(np.isinf(t)):
            print("Warning: Invalid translation vector, using MPJPE instead")
            return mpjpe(predicted, target)
        ...
        if np.any(np.isnan(predicted_aligned)) or np.any(np.isinf(predicted_aligned)):
            print("Warning: Invalid aligned prediction, using MPJPE instead")
            return mpjpe(predicted, target)

        # Return MPJPE
        error = np.mean(np.linalg.norm(predicted_aligned - target, axis=len(target.shape)-1), axis=1)

        # Check final error
        if np.any(np.isnan(error)) or np.any(np.isinf(error)):
            print("Warning: Invalid final error, using MPJPE instead")
            return mpjpe(predicted, target)

        return error

    except Exception as e:
        print(f"Warning: Exception in p_mpjpe calculation: {e}, using MPJPE instead")
        return mpjpe(predicted, target)
