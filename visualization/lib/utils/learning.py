...
def load_pretrained_weights(model, checkpoint_path):
    """
    智能加载 MotionBERT (DSTformer) 权重到 PoseCASTformer
    """
    import torch
    ...
    # 2. 定义层数限制 (只加载前2层到 independent_encoder)
    INDEPENDENT_DEPTH = 2
    ...
    for k, v in src_state.items():
        ...
        if k.startswith('joints_embed') or k.startswith('pos_embed') or k.startswith('temp_embed'):
            ...
            # 【Update Fix Start】: 修复 pos_embed 维度不匹配问题
            if 'pos_embed' in k:
                # 原始: [1, 17, 512], 目标: [1, 1, 17, 512]
                if v.ndim == 3 and target_state[new_k].ndim == 4:
                    # 在 dim=1 (Time维度) 插入一个维度
                    v = v.unsqueeze(1)
                    print(f"Auto-fixing shape for {k}: {v.shape}")
            # 【Update Fix End】

        # --- B. 映射 Independent Encoder (Layer 0 ~ N-1) ---
...
def load_backbone(args):
    ...
    if args.backbone=='DSTformer':
        ...
        # 打印一下，方便在日志中确认当前模型真正运行的模式
        print(f"初始化模型架构: PoseCASTformer, 实验模式: {ablation_mode}")
        ...
        model_backbone = PoseCASTformer(
            ...
            ablation_mode=ablation_mode  # 确保传递到 model_posecastformer.py
        )
