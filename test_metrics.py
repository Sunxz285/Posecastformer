...
from lib.model.loss import mpjpe, p_mpjpe   # 导入 p_mpjpe
...
def evaluate(args, model_pos, test_loader, datareader, checkpoint_path, log_path, config_path):
    ...
    results_mpjpe = {}
    results_mpjve = {}
    results_pampjpe = {}   # 新增：存储 PA-MPJPE（即 P-MPJPE）

    action_names = sorted(set(datareader.dt_dataset['test']['action']))
    for action in action_names:
        results_mpjpe[action] = []
        results_mpjve[action] = []
        results_pampjpe[action] = []   # 初始化
